...
export const Register = {
  observable(name, observable, actionsMap, storeName) {
    invariant(
      isObservable(observable),
      '%s should register observables but got %s',
      storeName,
      observable
    );
    ...
    warning(
      !actionsMap.has(name),
      '%s attempted to register, %s, which already exists',
      storeName,
      name
    );

    debug(
      '%s registering %s action',
      storeName,
      name
    );
  ...
  actions(actions, actionsMap, storeName) {
    invariant(
      typeof actions === 'object' &&
      actions instanceof Actions,
      '%s attempted to register non ThunderCats Actions object. Got %s',
      storeName,
      actions
    );
    let actionNames = actions.__getActionNames(actions);
    ...
    actionNames.map(name => {
      Register.observable(name, actions[name], actionsMap);
    });
...
class Store extends Rx.Observable {
  ...
  registerAction(name, observable) {
    Register.observable(name, observable, this._actions, this.displayName);
  }

  registerActions(actions) {
    Register.actions(actions, this._actions, this.displayName);
  }
  ...
  _initOperations() {
    ...
    let operations = [];
    this._actions.forEach((observable, name) => {
      debug('adding %s', name);
    ...
    this._operationsSubscription =
      Rx.Observable.merge(operations).subscribe(
  ...
  _notifyObservers() {
    ...
    this._observers.forEach((observer, uid) => {
  ...
  _applyOperation(value, operation) {
    if (operation.value) {
      ...
      return assign({}, value, operation.set);
    }
  }
