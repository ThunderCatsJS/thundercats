...
class Store extends Rx.Observable {
  ...
  _opsOnNext(operation) {
    ...
    invariant(
      typeof ops.value === 'object' ||
      typeof ops.transform === 'function' ||
      typeof ops.set === 'object',
      'invalid operation, ' +
    ...
    this.value = this._applyOperation(this.value, ops);
    ...
    if ('confirm' in ops) {
      ...
      invariant(
        isPromise(confirm) ||
        isObservable(confirm),
        'invalid operation, confirm should be a promise or observable,' +
        'given : %s',
        confirm
      );

      const confirmObs = isPromise(confirm) ?
        Rx.Observable.fromPromise(confirm) :
        confirm;
