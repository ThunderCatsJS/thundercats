...
export function RenderToString(cat, Component, ctx) {
  ...
  const Burrito = ContextWrapper.wrap(Component, cat);
...
export function fetch(path, fetchMap, stores) {
  ...
  let fetchCtx = Rx.Observable.from(fetchMap.values()).shareReplay();

  let storesObs = fetchCtx
    .pluck('store')
    .toArray();

  let fetchValues = fetchCtx
    .map(({ action, payload }) => ({ action, payload }));

  let waitForStores;
  storesObs
    .tap((arrayOfStores) => {
      debug('waiting for %s stores', arrayOfStores.length);
    })
    .subscribe(
      (arrayOfStores) => {
        waitForStores = waitFor(...arrayOfStores)
          .firstOrDefault()
          .shareReplay();
        waitForStores.subscribe();
      },
      (err) => {
        waitForStores = Rx.Observable.throw(err);
      });
