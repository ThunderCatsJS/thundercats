...
class Cat {
  ...
  _registerStore(name, ExtendStore, args) {
    ...
    this._stores.set(name, storeIns);
  }
  ...
  _registerActions(name, ExtendedActions, args) {
    ...
    debug('registering action %s', name);
    this._actions.set(name, actions);
  }
  ...
  serialize() {
    ...
    Rx.Observable.from(this._stores.values())
      .filter(store => { return !!store.displayName; })
      .filter(store => { return !!store.__value; })
      .map(store => {
        return { [store.displayName]: store.__value };
      })
      ...
      .subscribe(
        val => {
          ...
          dats = typeof val === 'string' ? val : '';
        },
        err => { debug('an error occured while stringifing stores', err); },
        () => { debug('deserialized completed'); }
      );
    return dats;
  }
  ...
  deserialize(stringyCatState) {
    ...
    let catState = JSON.parse(stringyCatState);
    invariant(
    ...
    Rx.Observable.from(this._stores.values())
      .map(store => {
        return {
          store,
          data: catState[store.displayName]
        };
      })
      .map(({ store, data }) => {
        if (typeof data === 'object') {
          ...
          debug(
            'deserialize found a store state that is not an object',
            data
          );
        }
        return true;
